...
<body class="page-header-fixed">
					...
					<div class="span12">
						<table border='1' class="table table-bordered table-hover">
							...
							<tr>
								<td id="eId"> 
									<c:if test="${exam!=null}">
										<c:out value="${exam.eId}"/>
									</c:if>
								</td>
								<td>
									<c:if test="${exam!=null}">
										<input type='text' id="eTitle" value="${exam.eTitle}">
									</c:if>
								</td>
								<td>
									<c:if test="${orderList!=null}">
										<c:forEach items="${orderList}" var="order">
											${order.qtTitle}:
											<select>
												<c:forEach begin="1" end="${totalEQT}" step="1" var="num">
													<c:if test="${num==order.qtOrder}">
														<option value="${order.qtId}" selected="selected">${num}</option>
													</c:if>
													<c:if test="${num!=order.qtOrder}">
														<option value="${order.qtId}">${num}</option>
													</c:if>
												</c:forEach>
											</select></br>
										</c:forEach>
						...
						<button class="btn btn-default" id="modifyButton">修改</button>
	...
	<script src="../../public/js/testPaperModify.js"></script>
